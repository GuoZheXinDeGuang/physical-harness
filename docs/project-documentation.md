...
**面板的写路径只有 brief 生命周期**：投 brief 走 `submit_brief`，或走 `submit_skill_plan`（规划面板的
“执行”按钮：服务端重新核验一张已校验、全部叶子有 binding 的 skill plan，再投一张**普通** task brief，
见 §6.1.2）；两者进的都是 runtime 校验过的 inbox，`cancel_brief` 是唯一的另一条写。规划本身
（`plan_skill_task`）是读：不执行、不落盘。**没有认证层**：`trusted-host` 防的是 DNS rebinding，不是身份；服务绑
...
```
pass       : 856 passed
skips      : 43 skipped   （这一轮的 checkout 没有封存 runs/ 证据：按 §3.3 多出 13 个
             读封存产物的跳过项；有 runs/ 时应为 30 skipped）
...
806 → 856 是 2026-09-01 skill-graph 规划闭环（§6.1.2）加的 50 个底座项：`test_unified_skill_graph.py`
（loader / 检索 / 展开）、`test_skill_planning.py`（假 DeepSeek 下的路由 / 校验 / binding 边界）、
`test_planning_faces.py`（MCP 与 CLI 两张脸、submit 拒绝与落盘、mocked 端到端）。
798 → 806 是再往前攒的 8 个底座项（7 个来自其间的提交，+1 是 grasp 谓词审计
这一轮的 `test_mission_kitchen_thaw.py::test_grasp_verify_is_secure_dz_shaped_not_the_bare_latch`）。

**全量对照（卡在场，robocasa 标记项排除）**：2026-09-02 实测 `886 passed, 17 skipped, 24 deselected`
（`pytest -m "not robocasa"`，harness `.venv`，无封存 runs/）。更早一次全量 `829 passed, 27 skipped`（2026-08-29 的 797 + 那一轮
...
Place、PressButton……）。今天有 binding 的只有静态技能库的 `navigate_to_object / pick / transport /
place_in`，且只在 `pack_all_robocasa` / `basket_smoke_vlm` 各自的场景与物体清单内。要让一个图技能变成
可执行，得有一张卡在 `[task_bindings.<task>]` 的 catalogue 里以**同名**声明它并给出 policy——展示别名
（`pick`→`Pick`）不算。
