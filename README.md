...
当前全量: **462 通过, 3 跳过**(跳过 = Mac 归档 campaign 与 BC 权重不在 git, 预期)。

底座快道(W6, 改底座前后各跑一遍、变差不许合入)= 隔离进程(robosuite 卡缺席)里 `pytest -m "not robosuite"`: **432 通过, 6 跳过, 27 弃**。快照格式与隔离跑法见 [docs/base-gate.md](docs/base-gate.md)。
...
- **驾驶舱 = ph-station(dsh fork)✅:** 骑标准 MCP 缝零 vendor 接 dsh(:3080), 七只只读工具逐字节等价 `board.store` + `submit_brief` 原子投递; 战报/演进/机箱/账本面板经 fork host BoardBridge(`/api/board/*` execFile→`board.storecli`, 逐字节等价)只读渲染, 报告另由 `python -m board.report` headless 出。**PH 上牌**: 品牌原生进 fork 源码(标题/PWA/favicon/slogan/wordmark), 上游 MIT 署名保留、`apps/cli` 改名 `@physical-harness/console` 且 private; 早期对 npx 安装副本打补丁的 `profiles/dsh/rebrand.sh` overlay 在 fork 达到对等后退役(见 docs/ph-station-design.md §6)。
