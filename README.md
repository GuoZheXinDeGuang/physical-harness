...
- **全量**(装了 robosuite 卡): **632 通过, 26 跳过**(跳过 = Mac 归档 campaign 与 BC 权重不在 git, 加 23 个 robocasa 卡/驱动/取景帧/mission 自证只在 robocasa venv 跑, 预期; robocasa venv 里 `-m robocasa` → 17 通过 + 6 xfail, xfail = 实测驱动诚实失败面 空载导航去微波炉(冰箱堵通道)/关门/从驻停位放置/seed4-5 假抓取闭锁, 加 steam_prep 的 "awaiting sink driver" 水龙头前沿)。
- **底座快道**(改底座前后各跑一遍、变差不许合入)= 隔离进程(robosuite 卡缺席)里
  `pytest -m "not robosuite"`: **601 通过, 29 跳过, 28 弃**。快照格式与隔离跑法见
