...
env the stage driver hands it (``bind(env, target=)``). Duck-typed to the
executor contract -- ``handshake`` / ``reset`` / ``act(obs)`` / ``done`` /
``diagnostics`` -- and mounted like any policy provider (``provider(**params)``
...
class _Handshake(dict):
    """The handshake record: a dict (what the workload seals via
    ``getattr(driver, "handshake")``) that also answers the contract's
    ``handshake()`` call."""
...
class GeometricGraspExecutor:
    ...
    def __init__(self, tunables: dict[str, float], target: str | None = None) -> None:
        ...
        self.target = target
        self._env: Any = None
        self.handshake = _Handshake(transport="inproc", ref=REF, checkpoint_sha=None,
                                    unverified=[], ok=True, tunables=dict(tunables))
        self.reset()
...
if __name__ == "__main__":
    ...
    assert ex.handshake()["transport"] == "inproc" and dict(ex.handshake)["ref"] == REF
