...
def _graph() -> dict:
    ...
    for obj in M.ITEMS:
        ...
        calls = (
            ("navigate", {"object": obj}),
            ("grasp", {"object": obj}),
            ("carry", {"object": obj, "target": target}),
            ("place", {"object": obj, "target": target}),
        )
...
def test_abstract_skill_resolves_to_robocasa_private_stage_name():
    ep, ctx = _ctx()
    node = {"id": "place-hot0", "skill": "place", "kind": "segment",
...
def _basket_graph() -> dict:
    ...
    for item in B.ITEMS:
        ...
        place = f"place-{item}"
        nodes.extend([
            {"id": pick, "skill": "grasp", "kind": "segment",
             "args": {"object": item}, "after": [previous] if previous else []},
            {"id": place, "skill": "place", "kind": "segment",
...
def test_basket_requirement_rejects_wrong_target_and_unordered_place():
    ...
    ok, msg = validate_plan(
        graph, B.CATALOGUE, B.ORACLES, requirements=B.PLANNING_CONTEXT)
    assert not ok and "requires 'place' after 'grasp'" in msg
...
def test_basket_abstract_calls_resolve_to_same_counter_driver_stages():
    ...
    pick = {"id": "pick-item0", "skill": "grasp", "kind": "segment",
            "args": {"object": "item0"}, "after": []}
    place = {"id": "place-item0", "skill": "place", "kind": "segment",
...
def _stack_graph() -> dict:
    return {
        ...
        "nodes": [
            {"id": "pick-cubeA", "skill": "grasp", "kind": "segment",
...
def test_stack_blocks_is_exact_two_skill_vlm_graph():
    assert set(S.CATALOGUE) == {"grasp", "place_on"}
    assert validate_plan(
