...
def pack_plan() -> dict:
    """The canonical pack_all_robocasa graph: per object navigate -> pick ->
    transport -> place_in, verify-covered, targets per planning_context."""
    from plugins.mission_pack_all import ITEMS, TARGET_BY_OBJECT
    nodes, verify, prev = [], [], None
    for obj in ITEMS:
        for skill in ("navigate_to_object", "pick", "transport", "place_in"):
            nid = f"{skill}-{obj}"
            args = {"object": obj}
            if skill in ("transport", "place_in"):
...
def test_prepare_coffee_routes_to_the_graph_with_a_compact_catalogue(endpoint):
    ...
    assert set(sent["catalogue"]) == set(names)
    assert sent["catalogue"]["CoffeeSetupMug"] == {} and sent["catalogue"]["Pick"] == {"object": "str"}
...
def test_pack_instruction_routes_to_the_executable_task_channel(endpoint):
    ...
    sent = _payload(_Server.requests[0])
    assert set(sent["catalogue"]) == {"navigate_to_object", "pick", "transport", "place_in"}
    assert sent["planning_context"]["required_per_object_order"] == [
...
def test_invented_or_mistyped_args_are_rejected(endpoint):
    ...
    assert "unknown arg 'object'" in _rejected(endpoint, plan, 8)
    plan = json.loads(json.dumps(COFFEE_PLAN))
    ...
    assert "arg 'object' must be str, got int" in _rejected(endpoint, plan, 9)
...
def test_coffee_chain_expands_server_side_and_is_planning_only(endpoint):
    ...
    assert out["composite_plan"]["plan"] == COFFEE_PLAN
...
def test_every_leaf_bound_makes_the_pack_plan_executable(endpoint):
    ...
    first = chain[0]
    assert first["label"] == "navigate_to_object.nav_hot1" and first["stage"] == "nav_hot1"
    ...
    assert chain[1]["after"] == ["navigate_to_object.nav_hot1"]
...
def test_binding_index_names_only_manifest_catalogue_skills():
    ...
    assert "pack_all_robocasa" in index["pick"] and "basket_smoke_vlm" in index["pick"]
