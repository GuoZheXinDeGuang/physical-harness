...
def test_fold_over_real_runs():
    ...
    # the three mounted skills + the 20 cards + the 10 capabilities are all present
    ...
    # inactive embodiment_libero third-simulator card).
    assert {STACK, ADC, EB} <= set(by_id)
    assert sum(n["kind"] == "package" for n in g["nodes"]) == 20
