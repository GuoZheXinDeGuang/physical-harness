...
def test_fold_over_real_runs():
    ...
    # the three mounted skills + the 19 cards + the 10 capabilities are all present
    ...
    # 19th: the inactive policy_vla_remote websocket transport card).
    assert {STACK, ADC, EB} <= set(by_id)
    assert sum(n["kind"] == "package" for n in g["nodes"]) == 19
