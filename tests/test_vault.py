...
def test_fold_over_real_runs():
    ...
    # the three mounted skills + the 14 cards + the 9 capabilities are all present
    ...
    # 14th: the M7 mission_kitchen_thaw persistent-episode mission on robocasa).
    assert {STACK, ADC, EB} <= set(by_id)
    assert sum(n["kind"] == "package" for n in g["nodes"]) == 14
