...
Round 35：**修复原语正式重开**（理由：实测 32.5pp 空间）。round 21/22 否掉的形状是在
**脚本策略**的失败状态上测的；克隆策略这 65 个修复缺口的形状从没测过。
按三段纪律：选择切片 → 不相交门禁 → 预注册确认性切片。
先做前提验证（round 21 的教训）：这 65 个失败的机械臂状态长什么样，再决定造什么。

**已排除：** 第三种策略形态用 `bc_h96.npz` —— 基线 2.5%，不是更弱而是不工作。
真正的第三种策略要训中间容量的 BC，成本不低，排在修复原语之后。
