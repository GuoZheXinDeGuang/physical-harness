...
Round 51：**把 held-out 判断数字写进报告第 06 节。** 那一节现在只有 dev 侧的对比，
而 dev 侧估计被证明抬高了 5-16pp。本轮已测得全新区块的数字：
**脚本 +29.5pp（65 修 6 破，p<1e-5）、克隆 +12.0pp（37 修 13 破，p=0.00094）**。
同时写进「样本内 vs 样本外收缩」那张表 —— 这是对自己新机制的诚实标注。
