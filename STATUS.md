...
Round 39：**报告更新。** 克隆策略现在有两区块复现：8000-8199 **+17.5pp** 与
8400-8599 **+16.5pp**，同一 bundle 只差 1.0pp —— 远比脚本策略那对（10pp）稳，
值得和区块噪声那一节放在一起讲。另加预筛的三臂结果（一个建好且对症的机制，
端到端测下来更差）与合取的负面结果（ambition critic 第 1 条的答案）。

**frontier 排序：** 1. beam 重测（round 24 理由已失效）→ 2. 真正的第三种策略（需训中间容量 BC）
→ 3. 沙箱化 critic 代码执行。
**已关闭：** 合取语言（round 37）、修复原语（round 35）、样本外预筛（round 38，端到端更差）。
