...
Round 15：**重跑脚本策略的 campaign**。加了运行时归约之后搜索空间变大了，
最佳候选从 `gripper_effort > 0.0447`（1.072）变成 `running_min(finger_gap) < 0.0129`（1.093）。
之前的 +27.5pp 是在旧语言下赚的，必须重新赚。

之后：**更新报告**。里面「基础策略是脚本，失败形态是我设计的」这条局限已经过时，
应改为「异质失败仍然更难治理：+12.0pp 对 +27.5pp」。
