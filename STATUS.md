...
- [!] **Round 13 关键负面结果**：克隆策略的异质失败上，搜索**零候选**，
      所有可观测特征峰值 σ ≤ 1.19（阈值 2.0）。**critic 表达力是绑定约束。**
      详见 docs/round13-bc-works.md
...
Round 14：**扩 critic 表达力**，这是 round 13 直接生成的 frontier。三条路，按可行性：
1. 失败聚类先行 —— 把异质失败分成同质簇再逐簇找 critic（Zetta 的 failure clustering，
   我们之前因为失败同质而跳过了）
2. 合取 critic —— `A < a AND B > b`，单特征阈值扩成小布尔表达式
3. 学出来的检测器 —— 但必须保持零特权和可审计

验收：在**克隆策略**上找到一条能过门禁的零特权 critic。
...
- 不要假设单谓词阈值够用：它要求失败在某一个可观测量上双峰。克隆策略的失败不是。
