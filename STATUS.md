...
Round 21：**接触反馈抓取原语**。这是 round 19 认定的唯一能抬天花板的路 ——
现在的修复用的是和策略同一套朴素合拢，39/42 够得着物体但抓不牢。
做法：一个用夹爪力/位置反馈闭环的合拢控制器，加进 repertoire，
按同样的纪律（选择切片 → 不相交门禁切片 → held-out）验证。

**注意这是往库里加一项独立能力，不是演化循环的产物** —— 这一点本身就是 round 19 的结论。
