...
Round 40：**beam 重测。** round 24 关掉 beam 的理由（「第二条 critic 无事可做」）
已被 round 32 推翻 —— 那是欠采样。现在规划器按实测产出率定样本量，
克隆策略能长到深度 2，beam 的前提（贪心提前收敛）第一次有条件被真正检验。
`governor/beam.py` 已有三路划分实现，held-out 用全新 8600-8799。
**注意：** beam 花 3 倍成本，必须和贪心同块对照，两臂都报。

**frontier 排序：** 1. beam 重测 → 2. 真正的第三种策略（需训中间容量 BC）
→ 3. 沙箱化 critic 代码执行。
