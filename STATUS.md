...
Round 17：**owner 通道 + recovery 库**，两者耦合，按顺序做：

1. **owner 通道**（先做，是前提）：rollout 已经知道每一步归策略还是归恢复动作
   （`recovery is not None`），记成一个通道，让搜索能限制在策略拥有的步上。
   不修的话第二代之后的搜索是在污染数据上做的 —— round 16 的 gen2 候选武装在 t=129，
   那是恢复程序内部。
2. **recovery 库**：现在只有一个脚本重抓。42/120 的失败被检测到却修不好，这是最大的一块。
   需要多个恢复动作 + 让 proposer 选择，而不只是调时长。
