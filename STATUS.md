...
gen2 候选符号对（3 修 0 破）但 p=0.25 —— 规划器的目标效应量是在脚本策略上定的，
用它规划克隆必然欠采样。**不要只给 gen2 加种子重跑（那是 optional stopping）**，
要改规划器输入后整个 campaign 从头跑。held-out 用全新的 8000-8199。

**重排后的 frontier：** 1. 克隆策略的第二条规则 → 1.5 修复原语（条件性重开，
round 21/22 否的是原语形状不是命题，且只在脚本策略的失败状态上测过）
→ 2. 第三种策略形态 → 3. 沙箱化 critic 代码执行。

**区块预算：** held-out 已用 1000-1999 五段 + 6000-6199 + **7000-7199（克隆，两臂各评一次，已花完）**；
选择区块用 2400-2549 / 2600-2749 / 2750-2899。下一个 held-out 用 8000-8199。
