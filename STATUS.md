...
Round 33：**报告更新。** 三处：(1) 克隆策略换成 +17.5pp / 两条规则链（区块 8000-8199），
round 16「只长一条规则」的结论已终结；(2) 新增区块噪声一段 ——
同一 bundle 两个 n=200 区块给 +30.5 / +20.5pp（2.0 SE），**所有单区块数字都该带 ±7pp**；
(3) 错误表加规划器那一条（把「介入」当成「改变结局」）。
