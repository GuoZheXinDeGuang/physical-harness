...
Round 48：**报告改三处。**
(1) 第 06 节「下界」改写为**「失败能否被选择性检测」** —— 给出 dart40(31.7%, 零规则)
    vs h256(32.5%, 有规则) 这一对几乎相同基线的对照，**推翻「策略太弱」的说法**；
(2) 新增判断测试的功效规划一段：**我加了一道测试却没给它功效**（同类错误第四次），
    且 n=60 的 +15.0% 在 n=140 下翻转为显著为负；
(3) 更新克隆数字并写明代价：新门禁把它从两条规则压回一条。
