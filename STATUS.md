...
Round 52：**用带 held-out 判断项的 campaign 重跑两个策略**，让产物里自带这一项。
现在的三区块数字是在 `heldout_vs_blind` 落地之前跑的（事后手工补测过，
但产物里没有）。脚本用 12600-12799，克隆用 12800-12999。
跑完核对：产物里的 `heldout_vs_blind` 应与 round 50 手测的 +29.5pp / +12.0pp 同量级。
