...
Round 41：**报告改两处。** (1) 克隆换成**三区块合并 +19.0pp**（117 修 3 破，n=600），
**删掉 round 39 写的「克隆复现比脚本紧得多」** —— 那是从 n=2 读出来的，
第三个区块给出 +23.0pp，跨度 6.5pp / sd 3.5pp，与脚本同量级；
(2) 第 09 节加 beam 重测（错误理由关闭 → 重开 → 修三次实验 → 仍是负面）。

**frontier 排序：** 1. 真正的第三种策略（需训中间容量 BC）→ 2. 沙箱化 critic 代码执行。
