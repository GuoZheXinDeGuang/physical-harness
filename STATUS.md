...
Round 47：**把下界定位得更准。** 现在只知道 15.8%（零规则）和 32.5%（两条规则）之间
有一条界。扫 σ ∈ {0.35, 0.40, 0.45} 造出中间基线的策略，各跑 campaign（新门禁），
找**能长出第一条真规则的最低基线**。这是唯一还能加新事实的便宜方向。

**区块预算：** held-out 已用到 9800-9999。下一个用 10000+。
**演示管线：** `governor/demos.py`，σ 是旋钮（0.15→0.30 有陡坎）。
