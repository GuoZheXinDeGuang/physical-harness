...
Round 28：**更新报告**。三处要改：
1. 第 0 节轨迹图加上「标定」这一段，并**区分两种性质** ——
   前四步是拆掉不该算数的机制（+50→+17.5），第五步是修好一个真正次优的东西（→+30.5）。
   现在的叙事「每次收紧数字就往下走」不完整。
2. band headline 换成 **+30.5pp**（全新区块 6000-6199，46.5%→77.0%）。
3. 第 2 节 campaign 表换成标定后这一次。

**区块预算：** held-out 已用 1000-1999 五段 + 6000-6199；选择区块用 2400-2549 / 2600-2749 / 2750-2899。
