...
Round 43：**Review 轮**（上次是 round 36，之后跑了 6 轮）。
重点查一类反复出现的错误：**把一次测量的适用范围写得比它实际的大**
（round 31 归因、round 41 离散度、round 42 感知侧 —— 三次同类）。
对全文做一次「这句话的作用域是什么」扫描：哪些结论只对一个策略/一个 bundle/一个区块成立。

**frontier 排序：** 1. Review（作用域扫描）→ 2. 真正的第三种策略（需训中间容量 BC）
→ 3. 沙箱化 critic 代码执行。
