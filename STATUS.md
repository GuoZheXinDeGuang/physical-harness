...
Round 18：**按世代递增 dev 样本量**。这是 round 17 直接得出的：
`rollout_count` 是常数会让后期世代系统性功效不足（残余效应量本来更小）。
实测 9/2 在 n=120 上 p=0.065，n=180 上就是 p=0.013。
做法：按残余失败数定样本量，或对每代做功效计算。

之后：**recovery 库**（round 16 的 42/120 仍然成立）。
