...
Round 44：**真正的第三种策略。** 训一个中间容量/数据量的 BC（目标基线 10-20%，
在 h96 的 2.5% 和 h256 的 32.5% 之间），跑完整 campaign + 三区块。
理由：这是唯一还能扩 claim 的方向，且不需要 gate。

**注意 round 34 的教训：** 先量基线再决定，2.5% 那种不是「更弱」是「不工作」。
