...
Round 42：**给脚本策略补第三个 held-out 区块**（用 8800-8999）。
理由：刚撤回的那个洞是「n=2 估不出离散度」，而**脚本策略现在正是 n=2**，
报告里已如实标注「它那个 10.0pp 同样不是离散度的估计」。补一个区块就能把
两个策略的离散度都变成真估计 —— 便宜、直接、补的是自己刚承认的洞。
bundle 已冻结（round 27/32 的单规则 g1），只需在新区块上评一次 + 消融。

**frontier 排序：** 1. 脚本补第三区块 → 2. 真正的第三种策略（需训中间容量 BC）
→ 3. 沙箱化 critic 代码执行。
