...
def run_task(task: str, seed: int, max_replans: int | None = None,
    ...
    returns ``{"status": "error"}`` before any submit. The submit's advisory
    ``warning`` (see submit_brief) rides onto whatever outcome comes back -- it
    is the same read-only key, carried so the reason survives the wait.
    """
    ...
    brief_id = submitted["submitted"]

    def _warned(out: dict) -> dict:
    ...
    while time.monotonic() - start < timeout_s:
        if (done_dir / brief_id).exists():
            return _warned(_outcome("done", brief_id, session_dir, baseline,
                                    round(time.monotonic() - start, 3)))
        if (failed_dir / brief_id).exists():
            return _warned(_outcome("failed", brief_id, session_dir, baseline,
                                    round(time.monotonic() - start, 3)))
        time.sleep(0.5)
    return _warned({"status": "timeout", "brief_id": brief_id,
